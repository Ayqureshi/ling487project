...
for d in range(1,9):
    ...
    for s in range(len(Segs)):
        HR.append(np.load('/Users/ameenqureshi/Desktop/487seg/' + 'HS_' + str(d) + '_' + Segs[s] + '.npy'))
    for token in range(tn):
        hr = []
        for s in range(len(Segs)):
            sample_size = min(100, HR[s].shape[2])  # Take 100 or the available number, whichever is smaller
            hr.append(HR[s][:,:,np.random.choice(HR[s].shape[2], sample_size, replace=False)])
        CD = np.empty((25,1024))
        for e in range(25):
            for v in range(1024):
